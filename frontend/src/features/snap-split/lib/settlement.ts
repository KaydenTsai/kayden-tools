import type {Bill, Expense, MemberSummary, SettlementResult, Transfer} from "@/features/snap-split/types/snap-split";

// ============================================================================
// SettledTransfer Key 格式（集中管理，避免分散各處）
// ============================================================================

const SETTLED_KEY_SEPARATOR = '::';

/**
 * 建立 settledTransfer key
 * @example createSettledKey('uuid1', 'uuid2') => 'uuid1::uuid2'
 */
export function createSettledKey(fromId: string, toId: string): string {
    return `${fromId}${SETTLED_KEY_SEPARATOR}${toId}`;
}

/**
 * 解析 settledTransfer key
 * @example parseSettledKey('uuid1::uuid2') => ['uuid1', 'uuid2']
 */
export function parseSettledKey(key: string): [string, string] | null {
    const parts = key.split(SETTLED_KEY_SEPARATOR);
    if (parts.length !== 2) return null;
    return [parts[0], parts[1]];
}

/**
 * 檢查 key 是否匹配（用於 startsWith 場景）
 */
export function matchSettledKeyPrefix(key: string, fromId: string, toId: string): boolean {
    return key.startsWith(createSettledKey(fromId, toId));
}
...
export function isTransferSettled(
    ...
    return settledTransfers?.includes(createSettledKey(fromId, toId)) ?? false;
}
