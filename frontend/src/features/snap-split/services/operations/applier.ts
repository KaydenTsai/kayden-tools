...
const logger = createLogger('Applier');
...
export function applyOperationToBill(bill: Bill, op: Operation): Bill {
    ...
    switch (opType) {
        ...
        case "MEMBER_REMOVE": {
            // 追蹤被刪除的成員 ID（只有有 remoteId 的才需要告知伺服器）
            const removedMember = bill.members.find(m => m.id === targetId);
            if (removedMember?.remoteId) {
                newBill.deletedMemberIds = [...(bill.deletedMemberIds || []), removedMember.remoteId];
            }
        ...
        case "EXPENSE_DELETE": {
            // 追蹤被刪除的費用 ID（只有有 remoteId 的才需要告知伺服器）
            const removedExpense = bill.expenses.find(e => e.id === targetId);
            if (removedExpense?.remoteId) {
                newBill.deletedExpenseIds = [...(bill.deletedExpenseIds || []), removedExpense.remoteId];
            }
        ...
        case "ITEM_DELETE":
            newBill.expenses = bill.expenses.map(e => {
                ...
                if (removedItem) {
                    // 追蹤被刪除的 Item ID（只有有 remoteId 的才需要告知伺服器）
                    const newDeletedItemIds = removedItem.remoteId
                        ? [...(e.deletedItemIds || []), removedItem.remoteId]
                        : (e.deletedItemIds || []);
                    return {
                        ...e,
