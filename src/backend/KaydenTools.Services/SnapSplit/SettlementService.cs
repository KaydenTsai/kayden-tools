...
public class SettlementService : ISettlementService
{
    ...
    public async Task<Result<SettlementResultDto>> CalculateAsync(Guid billId, CancellationToken ct = default)
    {
        ...
        if (bill == null)
        {
            return Result.Failure<SettlementResultDto>(ErrorCodes.BillNotFound, "Bill not found.");
        }
    ...
    public async Task<Result> ToggleSettledAsync(Guid billId, Guid fromMemberId, Guid toMemberId, CancellationToken ct = default)
    {
        // 檢查帳單是否存在
    ...
    private static SettlementResultDto CalculateSettlement(Bill bill, HashSet<(Guid, Guid)> settledSet)
    {
        ...
        decimal totalWithServiceFee = 0m;
        ...
        ref decimal totalWithServiceFee)
    {
        ...
        // 付款人增加實付金額
        if (summaryMap.TryGetValue(expense.PaidById.Value, out var payer))
        {
