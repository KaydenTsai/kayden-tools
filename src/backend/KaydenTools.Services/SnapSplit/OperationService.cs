...
public class OperationService : IOperationService
{
    ...
    public async Task<Result<OperationDto>> ProcessOperationAsync(OperationRequestDto request, Guid? userId, CancellationToken ct = default)
    {
        try
        {
            return await _unitOfWork.ExecuteInTransactionAsync(async () =>
            {
                ...
                if (bill == null)
                {
                    return Result.Failure<OperationDto>(ErrorCodes.BillNotFound, "Bill not found.");
                }
    ...
    public async Task<List<OperationDto>> GetOperationsAsync(Guid billId, long sinceVersion, CancellationToken ct = default)
    {
        var ops = await _unitOfWork.Operations.FindAsync(
            x => x.BillId == billId && x.Version > sinceVersion,
            orderBy: q => q.OrderBy(x => x.Version),
            ct: ct);
    ...
    private void ApplyOperationToSnapshot(Bill bill, Operation op)
    {
        ...
        switch (op.OpType)
        {
            ...
            case "MEMBER_UNCLAIM":
                ...
                {
                    if (memberToUnclaim.OriginalName != null)
                    {
                        memberToUnclaim.Name = memberToUnclaim.OriginalName;
                    }
            ...
            case "EXPENSE_ADD":
                var expense = new Expense
                {
                    ...
                    PaidById = payload.TryGetProperty("paidById", out var pId) && pId.ValueKind != JsonValueKind.Null ? pId.GetGuid() : null,
                };
            ...
            case "EXPENSE_UPDATE":
                ...
                if (exp != null)
                {
                    ...
                    if (payload.TryGetProperty("paidById", out var ePaidById)) exp.PaidById = ePaidById.ValueKind == JsonValueKind.Null ? null : ePaidById.GetGuid();
                }
                break;
            ...
            // Item 操作
            case "ITEM_ADD":
                var expenseForNewItem = bill.Expenses.FirstOrDefault(e => e.Id == (payload.TryGetProperty("expenseId", out var expId) ? expId.GetGuid() : Guid.Empty));
                if (expenseForNewItem != null)
                {
                    var newItem = new ExpenseItem
                    {
                        ...
                        Amount = payload.GetProperty("amount").GetDecimal(),
                        PaidById = payload.TryGetProperty("paidById", out var itemPaidById) && itemPaidById.ValueKind != JsonValueKind.Null
            ...
            case "ITEM_UPDATE":
                ...
                {
                    if (payload.TryGetProperty("name", out var itemName)) itemToUpdate.Name = itemName.GetString() ?? itemToUpdate.Name;
                    if (payload.TryGetProperty("amount", out var itemAmount)) itemToUpdate.Amount = itemAmount.GetDecimal();
                    if (payload.TryGetProperty("paidById", out var newPaidById)) itemToUpdate.PaidById = newPaidById.ValueKind == JsonValueKind.Null ? null : newPaidById.GetGuid();
                }
                break;
            ...
            case "SETTLEMENT_MARK":
                    ...
                    payload.TryGetProperty("toMemberId", out var sToId))
                {
                    ...
                    // 檢查是否已存在
                    var existing = bill.SettledTransfers.FirstOrDefault(
            ...
            case "SETTLEMENT_UNMARK":
                ...
                {
                    var existing = bill.SettledTransfers.FirstOrDefault(
                        s => s.FromMemberId == uFromId.GetGuid() && s.ToMemberId == uToId.GetGuid());
                    if (existing != null)
                    {
                        bill.SettledTransfers.Remove(existing);
                    }
                }
                break;
