...
public class AuthService : IAuthService
{
    ...
    public async Task<Result<AuthResultDto>> LoginWithLineAsync(string code, string? state,
        CancellationToken ct = default)
    {
        // Exchange code for token
        var tokenResponse = await ExchangeLineCodeAsync(code, ct);
        if (tokenResponse == null)
            return Result.Failure<AuthResultDto>(ErrorCodes.InvalidCredentials,
    ...
    public async Task<Result<AuthResultDto>> RefreshTokenAsync(string refreshToken, CancellationToken ct = default)
    {
        ...
        if (token == null) return Result.Failure<AuthResultDto>(ErrorCodes.InvalidToken, "Invalid refresh token.");
        ...
        if (user == null) return Result.Failure<AuthResultDto>(ErrorCodes.NotFound, "User not found.");
    ...
    private async Task<LineTokenResponse?> ExchangeLineCodeAsync(string code, CancellationToken ct)
    {
        ...
        try
        {
            ...
            return await response.Content.ReadFromJsonAsync<LineTokenResponse>(ct);
        }
        catch
        {
            return null;
