...
public class ShortUrlService : IShortUrlService
{
    ...
    #region IShortUrlService Members

    public async Task<Result<ShortUrlDto>> CreateAsync(CreateShortUrlDto dto, Guid? ownerId,
        CancellationToken ct = default)
    {
        // 驗證 URL 格式
        if (!Uri.TryCreate(dto.OriginalUrl, UriKind.Absolute, out var uri) ||
            (uri.Scheme != "http" && uri.Scheme != "https"))
            return Result.Failure<ShortUrlDto>(ErrorCodes.InvalidUrlFormat,
                "Invalid URL format. Only http and https are allowed.");
        ...
        if (dto.ExpiresAt.HasValue)
        {
            ...
            if (dto.ExpiresAt.Value > maxExpiration)
                return Result.Failure<ShortUrlDto>(ErrorCodes.ExpirationExceeded,
                    $"Expiration cannot exceed {_settings.MaxTtlDays} days.");
        ...
        if (!string.IsNullOrEmpty(dto.CustomCode))
        {
            if (!IsValidCustomCode(dto.CustomCode))
                return Result.Failure<ShortUrlDto>(ErrorCodes.InvalidShortCode,
                    "Custom code contains invalid characters. Use only letters, numbers, and hyphens.");
    ...
    public async Task<Result<ShortUrlDto>> GetByIdAsync(Guid id, CancellationToken ct = default)
    {
        ...
        if (shortUrl == null) return Result.Failure<ShortUrlDto>(ErrorCodes.ShortUrlNotFound, "Short URL not found.");
    ...
    public async Task<Result<IReadOnlyList<ShortUrlSummaryDto>>> GetByOwnerIdAsync(Guid ownerId,
        CancellationToken ct = default)
    {
    ...
    public async Task<Result<ShortUrlDto>> UpdateAsync(Guid id, UpdateShortUrlDto dto, Guid? userId,
        CancellationToken ct = default)
    {
        var shortUrl = await _unitOfWork.ShortUrls.GetByIdAsync(id, ct);
        if (shortUrl == null) return Result.Failure<ShortUrlDto>(ErrorCodes.ShortUrlNotFound, "Short URL not found.");
        ...
        if (dto.IsActive.HasValue) shortUrl.IsActive = dto.IsActive.Value;
    ...
    {
        var shortUrl = await _unitOfWork.ShortUrls.GetByIdAsync(id, ct);
        if (shortUrl == null) return Result.Failure(ErrorCodes.ShortUrlNotFound, "Short URL not found.");
    ...
    public async Task<Result<string>> ResolveAsync(string shortCode, CancellationToken ct = default)
    {
        ...
        if (shortUrl == null) return Result.Failure<string>(ErrorCodes.ShortUrlNotFound, "Short URL not found.");
    ...
    public async Task<Result<UrlStatsDto>> GetStatsAsync(Guid id, DateOnly? from, DateOnly? to,
        CancellationToken ct = default)
    {
        var shortUrl = await _unitOfWork.ShortUrls.GetByIdAsync(id, ct);
        if (shortUrl == null) return Result.Failure<UrlStatsDto>(ErrorCodes.ShortUrlNotFound, "Short URL not found.");
    ...
    private async Task<string> GenerateUniqueShortCodeAsync(CancellationToken ct)
    {
        ...
        for (var attempt = 0; attempt < 10; attempt++)
        {
            ...
            if (!await _unitOfWork.ShortUrls.ShortCodeExistsAsync(code, ct)) return code;
