...
public class MemberService : IMemberService
{
    ...
    public async Task<Result<MemberDto>> CreateAsync(Guid billId, CreateMemberDto dto, CancellationToken ct = default)
    {
        ...
        if (bill == null) return Result.Failure<MemberDto>(ErrorCodes.BillNotFound, "Bill not found.");
    ...
    public async Task<Result<MemberDto>> UpdateAsync(Guid id, UpdateMemberDto dto, CancellationToken ct = default)
    {
        ...
        if (member == null) return Result.Failure<MemberDto>(ErrorCodes.MemberNotFound, "Member not found.");
    ...
    {
        var member = await _unitOfWork.Members.GetByIdAsync(id, ct);
        if (member == null) return Result.Failure(ErrorCodes.MemberNotFound, "Member not found.");
    ...
    /// 認領成員
    /// </summary>
    public async Task<Result<ClaimMemberResultDto>> ClaimAsync(Guid memberId, Guid userId, ClaimMemberDto dto,
        CancellationToken ct = default)
    {
        var member = await _unitOfWork.Members.GetByIdAsync(memberId, ct);
        if (member == null) return Result.Failure<ClaimMemberResultDto>(ErrorCodes.MemberNotFound, "Member not found.");

        // 檢查成員是否已被認領
        if (member.LinkedUserId.HasValue)
            return Result.Failure<ClaimMemberResultDto>(ErrorCodes.MemberAlreadyClaimed,
                "Member is already claimed by another user.");
        ...
        if (user == null) return Result.Failure<ClaimMemberResultDto>(ErrorCodes.NotFound, "User not found.");
    ...
    public async Task<Result> UnclaimAsync(Guid memberId, Guid userId, CancellationToken ct = default)
    {
        ...
        // 檢查成員是否已被認領
        if (!member.LinkedUserId.HasValue) return Result.Failure(ErrorCodes.MemberNotClaimed, "Member is not claimed.");
        ...
        if (!isOwner && !isClaimer)
            return Result.Failure(ErrorCodes.UnauthorizedUnclaim,
                "Only the claimer or bill owner can unclaim this member.");
