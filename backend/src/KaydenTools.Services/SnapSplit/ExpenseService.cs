...
public class ExpenseService : IExpenseService
{
    ...
    public async Task<Result<ExpenseDto>> GetByIdAsync(Guid id, CancellationToken ct = default)
    {
        ...
        if (expense == null) return Result.Failure<ExpenseDto>(ErrorCodes.ExpenseNotFound, "Expense not found.");
    ...
    public async Task<Result<ExpenseDto>> CreateAsync(Guid billId, CreateExpenseDto dto, CancellationToken ct = default)
    {
        ...
        if (bill == null) return Result.Failure<ExpenseDto>(ErrorCodes.BillNotFound, "Bill not found.");
        ...
        foreach (var participantId in dto.ParticipantIds)
            if (!memberIds.Contains(participantId))
                return Result.Failure<ExpenseDto>(ErrorCodes.MemberNotFound,
                    $"Participant {participantId} not found in this bill.");
        ...
        if (dto.IsItemized && dto.Items != null)
            foreach (var itemDto in dto.Items)
            {
                ...
                if (itemDto.PaidById.HasValue && !memberIds.Contains(itemDto.PaidById.Value))
                    return Result.Failure<ExpenseDto>(ErrorCodes.MemberNotFound,
                        $"Item payer {itemDto.PaidById} not found in this bill.");

                // 驗證細項參與者
                foreach (var itemParticipantId in itemDto.ParticipantIds)
                    if (!memberIds.Contains(itemParticipantId))
                        return Result.Failure<ExpenseDto>(ErrorCodes.MemberNotFound,
                            $"Item participant {itemParticipantId} not found in this bill.");
    ...
    public async Task<Result> DeleteAsync(Guid id, CancellationToken ct = default)
    {
        ...
        if (expense == null) return Result.Failure(ErrorCodes.ExpenseNotFound, "Expense not found.");
    ...
    private static ExpenseDto MapToDto(Expense expense)
    {
        return new ExpenseDto(
            ...
            expense.IsItemized
                ? expense.Items.Select(i => new ExpenseItemDto(
                    i.Id,
                    i.Name,
                    i.Amount,
                    i.PaidById,
